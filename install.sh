...
if [ ! -f "$INSTALL_DIR/.env" ]; then
  ...
  fi
  {
    ...
    printf 'CHINAZ_SUPPLEMENT_MAX_DELAY=8s\n'
    printf 'CHINAZ_SUPPLEMENT_COOLDOWN=15m\n'
    printf 'SCRAPE_USER_AGENT="seo-monitor/1.0 (daily metrics collector; contact your administrator)"\n'
    printf 'SCRAPE_TIMEOUT=25s\n'
    printf 'SCRAPE_MIN_DELAY=10s\n'
    printf 'SCRAPE_MAX_DELAY=20s\n'
    printf 'SCRAPE_RETRIES=2\n'
